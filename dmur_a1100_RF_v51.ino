...
1x NRF24L01 2.4ghz radio transciever and associated base module which regulates input voltages
*/
...
2016-03-14: Added RF 24L01 Module
*/
...
// include radio libraries
#include <SPI.h>
#include <nRF24L01.h>
#include <RF24.h>
#include <RF24_config.h>

// define RF module object
RF24 radio(9,10);
const uint64_t pipe0 = 0xE8E8F0F0E1LL;  // define channel of 'pipe 0' (the main broadcast channel for the sending module)
//const uint64_t pipe1 = 0xE8E8F0F0E0LL;
//const uint64_t pipe2 = 0xE8E8F0F0DFLL;


int prevhour = 99; // Used to store previous hour value to determine when we have started a new day
...
const int publishinterval = 1; // interval in SECONDS that data should be displayed (or pushed to a display device)
...
// define a structure for the information sent through RF payloads
typedef struct{
  char Header;
  int Month;
  int Day;  
...
const int importled = 6; // LED used when pulsing imports
const int exportled = 7; // LED used when pulsing exports
const int errorled = 5; // LED used for status information. I will illuminate this if an error occurs, suggesting to the operator to check the hardware/software.
...
// flash error led to indicate startup
...
if ((currenthour < 1) && (prevhour == 23) && (imports > 0)) // determine if it is past midnight and we have data from the meter
{
...
{
readRTC(); // substitute readRTC or noRTC depending whether you included an RTC module
...
of more than 50kw (50000)is unrealistic for a domestic environment */
    if (powercalc < 50000)
      {
      power = powercalc;
...
if (debugmode == true)
  {
  Serial.println("check times debug");
  }
...
// prepare average available power
        int avgpower3 = ( ( powersample[0] + powersample[1] + powersample[2] ) / 3 ); // average available energy for the last 3 samples
        int avgpower5 = ( ( powersample[0] + powersample[1] + powersample[2] + powersample[3] + powersample[4]  ) / 5 ); // average available energy for the last 5 samples


// populate the radio packet variables
payload.Header = '>';
...
payload.Power = power;
payload.Freepower = freepower;
payload.Avgpower = avgpower5;
payload.Timeup = (millis() / 60000);
payload.Trailer = '<';
...
// check whether its time to push stat updates.
if ( ( (millis() - lastUpload) > (uploadinterval * 60000) ) && imports > 0) // if its been more than the specified interval since we last published current values
   {
...
void readRTC()
{
  ...
  if (RTC.read(tm)) 
  {
    ...
    prevhour = currenthour; // set hour from last read to 'prevhour'
